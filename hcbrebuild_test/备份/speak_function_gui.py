...
def build_speak_lua(cfg: SpeakConfig, tmp: bytes | None = None) -> str:
    ...
    lines.append(f"  -- 固定部分：名字栏样式/角色显示编号")
    lines.append(f"  G[227] = {cfg.style_id}")
    lines.append(f"  __ret = f_{cfg.setup_style_addr:08X}(G[227])  -- SetupSpeakerNameStyle")
    if cfg.enable_direct_state_set:
...
class SpeakBuilderApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        fields = [
            ...
            ("style_id", "G[227] 样式ID"), ("state_global", "状态变量 G[]"), ("property_id", "属性/角色ID"),
